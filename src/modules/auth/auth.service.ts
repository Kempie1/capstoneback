...
export class AuthService {
  ...
  async requestResetPassword(body: ResetPasswordDto) {
    ...
    if (!user) {
      ...
      const passwordResetData = await this.createPasswordResetToken(user);
      const { tableId, token } = passwordResetData;
      const baseurl = this.configService.get<string>('FRONTEND_URL');
      const resetLink =
        baseurl +
        `/forgot-password/reset-password?token=${token}&id=${tableId}`;
  ...
  async completeResetPassword(body: CompleteResetPasswordDto) {
    ...
    const validToken = await this.verifyResetToken(
      { token: token, id: id },
      passwordResetEntity,
    );
    if (validToken) {
