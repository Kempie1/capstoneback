...
export class CartService {
    ...
    async addProductToCart(req, body: AddToCartDto) {
        ...
        let cartItem = await this.cartItemRepository.findOne({ where: { id: body.product, shoppingCart: cart  } });
        ...
        //if it is not, add the product to the cart
            ...
            let product = await this.productsRepository.findOne({ where: { id: body.product }});
            console.log(product);
            if (!product) {
    ...
    async removeProductFromCart(req, body: RemoveFromCartDto) {
        ...
        let cart = await this.shoppingCartRepository.findOne({where:{user: req.user.sub}, relations: ['cartItems']});
        //check if the product is already in the cart
        let product = await this.cartItemRepository.findOne({ where: { id: body.product, shoppingCart: cart  } });
